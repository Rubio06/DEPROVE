    ...
    excel_file.addEventListener('change', (event) => {
        ...
        reader.onload = function(event) {
            ...
            if (sheet_data.length > 0) {
                id++;
                max = 299;
                    sql = "INSERT INTO documentosCaja (SEC,cliente,linea, importe, saldo, inicial,tipo,fecha,empresa,estado1,estado2) VALUES ";
                ...
                if (sql != "INSERT INTO documentos (cliente, importe, saldo, inicial,tipo,fecha,empresa,estado1,estado2) VALUES ") {
    ...
    excel_file.addEventListener('change', (event) => {
        ...
        reader.onload = function(event) {
            ...
            if (sheet_data.length > 0) {
                id++;
                max = 299;
                    sql = "INSERT INTO comisiones (distribuidor,cliente, telefono, cf, tipoplan,transaccion,comisiontotal,oc,fechacarga,sec,dnicliente) VALUES ";
                for (var row = 0; row < sheet_data.length; row++) {
                    arraycelda = [];
                    if (max < row || id == sheet_data.length) {
                        tr = document.createElement("tr");
                ...
                if (sql != "INSERT INTO comisiones (distribuidor,cliente, telefono, cf, tipoplan,transaccion,comisiontotal,oc,fechacarga,sec,dnicliente) VALUES ") {
                    var xmlhttp = new XMLHttpRequest();
                    xmlhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            console.log(this.response);
